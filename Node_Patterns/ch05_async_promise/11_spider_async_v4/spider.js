...
async function download (url, filename) {
    ...
    const { text: content } = await superagent.get(url) // destructuring
    await mkdirpPromises(dirname(filename))
    await fsPromises.writeFile(filename, content)
    console.log(`Downloaded and saved: ${url}`)
    return content // no nesting
}
...
async function spiderLinks (currentUrl, content, nesting, queue) {
    ...
    return Promise.all(promises) // asynchronously grab all
}

const spidering = new Set()

export async function spiderTask (url, nesting, queue) {
