...
const server = fastify({ logger: true }) // fastify server instance

server.register(fastifyStatic, { // load public/main.js
  root: resolve(__dirname, '..', 'public'),
  prefix: '/public/'
})

server.get('*', async (req, reply) => { // catch all get requests and point to React router
  ...
  `
  const content = reactServer.renderToString(serverApp) // generate HTML code
