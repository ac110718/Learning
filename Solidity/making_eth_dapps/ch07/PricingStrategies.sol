...
// concrete instance of SimpleCrowdSale.. which has abstract methods for Funding Strategy (cap vs no limit) and pricing (fix vs. tranche)
// hierarchy is define pricing first.. then define funding strategy..
...
contract FixedPricingCrowdSale is SimpleCrowdSale {
    constructor(uint256 _startTime, uint256 _endTime, uint256 _weiTokenPrice, uint256 _etherInvestmentObjective, uint256 _fundingCap)
        SimpleCrowdSale(_startTime, _endTime, _weiTokenPrice, _etherInvestmentObjective) payable public {
    }
